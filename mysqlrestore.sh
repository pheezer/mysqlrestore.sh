...
mysql --socket=$SOCKET -Ns -e'show databases;'| perl -ne 'print unless /\b(mysql|psa|horde|atmail|information_schema|apsc|sitebuilder.*|phpmyadmin.*)\b/'|while read x
	do
		echo -n "Dumping $x ...";
		 mysqldump --add-drop-table --skip-lock-tables --single-transaction --socket=$SOCKET $x > "$RESTOREDIR"/$x.sql;
		echo "Finished.";
	done
